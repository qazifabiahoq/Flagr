...
1. A deterministic rule engine that checks the transaction against a library of 33 regulation-mapped compliance rules across seven categories, evaluated in under a millisecond.
...
Pawn shops, check cashing outlets, and payday lenders are classified as money service businesses under BSA 31 CFR 1022 and are disproportionately represented in FinCEN SAR filings involving currency structuring and smurfing operations. The combination of cash-intensive operations, limited customer due diligence at point of transaction, and high turnover makes these businesses a preferred placement-stage vehicle. Precious metals, coin, and gemstone dealers are flagged under FinCEN FIN-2018-G001 guidance because high-value portable commodities allow large sums to be converted into assets with no serial numbers, no counterparty records, and no cross-border reporting requirement. The FATF has specifically identified gold and diamond dealers as vulnerable to placement-stage laundering because the asset can be purchased with illicit cash and later sold through legitimate secondary markets.
...
Trade-based money laundering is identified when a large payment flows to an import/export, freight, logistics, or wholesale merchant in a high-risk jurisdiction. TBML is the mechanism most commonly cited in FATF typology reports for moving value across borders without triggering currency reporting requirements. It works by over-invoicing or under-invoicing goods and services to create a legitimate paper trail for an otherwise unexplained cross-border transfer. The FATF explicitly addresses TBML in Recommendation 14 and FinCEN issued specific guidance on the typology in FIN-2010-A008. A separate rule addresses transactions that combine high value with a jurisdiction carrying elevated Politically Exposed Person risk as measured by the Basel AML Index and Transparency International's Corruption Perceptions Index. PEP-linked flows carry a heightened risk of bribery proceeds, embezzlement of public funds, and state-sponsored financial crime. The FFIEC Customer Due Diligence requirements mandate enhanced due diligence for all transactions involving PEPs and their immediate associates, with specific attention to jurisdiction of origin.
