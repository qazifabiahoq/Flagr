...
export const FRAUD_RULES = [
  ...
  // Additional MERCHANT rules
  {
    id: 'MER-008',
    category: 'MERCHANT',
    name: 'Pawn Shop, Check Cashing, or Payday Lender',
    description:
      'The transaction involves a pawn shop, check cashing outlet, or payday lending service. These businesses are classified as money service businesses under BSA and are disproportionately represented in currency layering and structuring cases. The FinCEN SAR database consistently identifies this merchant category in smurfing operations.',
    severity: 'medium',
    regulation: 'BSA 31 CFR 1022',
    check: (txn) => {
      const m = (txn.merchant || '').toLowerCase();
      return (
        m.includes('pawn') ||
        m.includes('check cash') ||
        m.includes('cheque cash') ||
        m.includes('payday') ||
        m.includes('cash advance') ||
        m.includes('title loan')
      );
    },
  },
  {
    id: 'MER-009',
    category: 'MERCHANT',
    name: 'Precious Metals or Gems Dealer',
    description:
      'The transaction is directed to a precious metals, coins, or gemstone dealer. High-value portable commodities such as gold, silver, and diamonds are a well-documented method for converting currency into untraceable assets. FinCEN and the FATF have identified these dealers as vulnerable to placement-stage money laundering.',
    severity: 'medium',
    regulation: 'FinCEN FIN-2018-G001',
    check: (txn) => {
      const m = (txn.merchant || '').toLowerCase();
      return (
        m.includes('gold') ||
        m.includes('silver') ||
        m.includes('bullion') ||
        m.includes('precious metal') ||
        m.includes('gem') ||
        m.includes('diamond') ||
        m.includes('jewel') ||
        m.includes('coin dealer')
      );
    },
  },
  ...
  // Additional AML rules
  {
    id: 'AML-006',
    category: 'AML',
    name: 'Trade-Based Money Laundering Indicator',
    description:
      'The transaction pattern is consistent with trade-based money laundering: an unusually large or round-dollar payment to an import/export, freight, logistics, or wholesale merchant, particularly one in a high-risk jurisdiction. TBML is the FATF\'s most commonly identified mechanism for moving value across borders without detection.',
    severity: 'high',
    regulation: 'FATF Recommendation 14 / FinCEN FIN-2010-A008',
    check: (txn) => {
      const m = (txn.merchant || '').toLowerCase();
      const amount = parseFloat(txn.amount);
      const isTradeMerchant =
        m.includes('import') ||
        m.includes('export') ||
        m.includes('freight') ||
        m.includes('logistics') ||
        m.includes('wholesale') ||
        m.includes('trading co') ||
        m.includes('supply co');
      const isRiskyGeo = ['CN', 'HK', 'AE', 'PH', 'NG', 'MX'].includes(txn.location);
      return isTradeMerchant && isRiskyGeo && amount >= 2000;
    },
  },
  {
    id: 'AML-007',
    category: 'AML',
    name: 'Politically Exposed Person Jurisdiction Correlation',
    description:
      'The transaction combines a high-value amount with a jurisdiction that has a high prevalence of Politically Exposed Persons and public corruption risk, as classified by the Basel AML Index and Transparency International. PEP-correlated transactions carry elevated risk of bribery, embezzlement, and state-sponsored money laundering under the FFIEC\'s Customer Due Diligence requirements.',
    severity: 'high',
    regulation: 'FFIEC CDD Requirements / Basel AML Index',
    check: (txn) => {
      const amount = parseFloat(txn.amount);
      const pepCorrelatedCountries = ['NG', 'UA', 'RU', 'VN', 'PH', 'KZ', 'AZ', 'BY'];
      return amount >= 5000 && pepCorrelatedCountries.includes(txn.location);
    },
  },
  ...
  // Additional TIME rules
  {
    id: 'TIME-003',
    category: 'TIME',
    name: 'Off-Hours High-Value Wire',
    description:
      'A high-value transaction (over $3,000) occurring outside normal banking hours (5 PM to 8 AM). While the off-hours signal alone is a medium-severity indicator, the combination of high value and off-hours timing significantly elevates the risk profile. Authorized wire transfers of this size are almost always initiated during business hours.',
    severity: 'high',
    check: (txn) => {
      const hours = Math.floor((parseFloat(txn.time_seconds) || 0) / 3600);
      const isOffHours = hours >= 17 || hours < 8;
      const isHighValue = parseFloat(txn.amount) >= 3000;
      return isOffHours && isHighValue;
    },
  },

  // Additional GEOGRAPHIC rules
  {
    id: 'GEO-005',
    category: 'GEOGRAPHIC',
    name: 'Hawala-Linked Jurisdiction',
    description:
      'The transaction originates from or is directed to a country where hawala and informal value transfer systems are prevalent. Hawala operates outside the formal banking system with no paper trail and is explicitly flagged under FATF Recommendation 14 as a high-risk informal transfer mechanism used extensively in terror financing and sanctions evasion.',
    severity: 'high',
    regulation: 'FATF Recommendation 14',
    check: (txn) => ['PK', 'AF', 'SO', 'YE', 'IQ', 'LB'].includes(txn.location),
  },
];
